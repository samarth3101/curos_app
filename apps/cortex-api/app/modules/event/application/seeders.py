...
async def seed_event_workflow(organization_id: str, actor_id: str, workflow_service: WorkflowService) -> str:
    ...
    state_submitted = await workflow_service.add_state(organization_id, def_id, "Submitted", "submitted", WorkflowStateType.NORMAL)
    state_approved = await workflow_service.add_state(organization_id, def_id, "Approved", "approved", WorkflowStateType.NORMAL)
    state_published = await workflow_service.add_state(organization_id, def_id, "Published", "published", WorkflowStateType.NORMAL)
    state_ongoing = await workflow_service.add_state(organization_id, def_id, "Ongoing", "ongoing", WorkflowStateType.NORMAL)
    state_completed = await workflow_service.add_state(organization_id, def_id, "Completed", "completed", WorkflowStateType.FINAL)
    state_archived = await workflow_service.add_state(organization_id, def_id, "Archived", "archived", WorkflowStateType.FINAL)

    # Add Transitions
    await workflow_service.add_transition(organization_id, def_id, state_draft.id, state_submitted.id, "submit", "event.submit")
    await workflow_service.add_transition(organization_id, def_id, state_submitted.id, state_approved.id, "approve", "event.approve")
    await workflow_service.add_transition(organization_id, def_id, state_submitted.id, state_draft.id, "reject", "event.approve")
    ...
    await workflow_service.add_transition(organization_id, def_id, state_completed.id, state_archived.id, "archive", "event.manage")
