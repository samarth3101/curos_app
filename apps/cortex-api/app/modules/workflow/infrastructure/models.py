...
class WorkflowDefinitionModel(Base):
    ...
    status = Column(
        SQLEnum(WorkflowDefinitionStatus), nullable=False, default=WorkflowDefinitionStatus.DRAFT
    )
...
class WorkflowTransitionModel(Base):
    ...
    from_state_id = Column(
        String(36), ForeignKey("workflow_states.id", ondelete="CASCADE"), nullable=False
    )
    to_state_id = Column(
        String(36), ForeignKey("workflow_states.id", ondelete="CASCADE"), nullable=False
    )
...
class WorkflowInstanceModel(Base):
    ...
    status = Column(
        SQLEnum(WorkflowInstanceStatus), nullable=False, default=WorkflowInstanceStatus.ACTIVE
    )
...
class WorkflowTaskModel(Base):
    ...
    assigned_user_id = Column(
        String(36), ForeignKey("identity_users.id", ondelete="SET NULL"), index=True, nullable=True
    )
...
class WorkflowExecutionModel(Base):
    ...
    )  # Store state keys, not IDs, since it's immutable history
